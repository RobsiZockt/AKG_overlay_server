...
let leftfade = $state("");
let rightfade = $state("");
let {trigger = $bindable(true), team1,team2,select_map} = $props();

$effect(()=>{
  if(select_map==undefined||team1==undefined||team2==undefined)
    return;
  getPred(select_map,team1,team2);
})



async function getPred(map,t1,t2) {
    try {
      const res = await fetch(`/api/pred/predict_outcome/${t1}/${t2}/${map}`, {
        method: "GET",
...
{#if trigger == true}
		...
		{#if predData != {}}
			...
			<div class="w-full h-full flex-col bg-[#000000ee] border-4 border-black">
				<!--Name HEader-->
				...
				<div class="w-full h-7/12 items-center justify-center flex p-4">
					<div class="w-1/3 h-full items-center justify-start flex-col"> 
						<img src='{predData.team1_logo}' alt="" class="h-auto aspect-square object-cover"/>
					...
					<img src='{predData.team2_logo}' alt="" class="h-auto aspect-square object-cover"/>
						<div class="w-auto h-auto flex justify-center">   
